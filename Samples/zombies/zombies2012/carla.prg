...
private
	...
	golpeado;				// si la estan golpeando a carla
...
/* -------------------------------------------------------------------------- */
		...
		IF ( SOBRE_SUELO )
			...
			// CAMINA DERECHA
			ELSEIF ( jkeys_state[ _JKEY_RIGHT     ] )
			...
			// salto chongo
			IF  ( jkeys_state[ _JKEY_Y         ] AND SALTO_POSIBLE)
				salto_posible = FALSE;
			...
			IF ( SALTO_POSIBLE == FALSE AND NOT jkeys_state[ _JKEY_Y         ] )
				salto_posible = TRUE;
			END
...
/* -------------------------------------------------------------------------- */
		...
		IF ( (invencible == false) and (collision (type zombie_proyectil ) or collision (type zombie) ) )
