...
begin
	...
	loop
		...
		if ( jkeys_state[ _JKEY_B         ] and cambio_permitido)
		...
		elseif ( jkeys_state[ _JKEY_A         ] and cambio_permitido)
			cambio_permitido = false;
			cambiar_arma(1);
		...
		if ( not jkeys_state[ _JKEY_B         ] and not jkeys_state[ _JKEY_A         ] and cambio_permitido == false)
		...
		if ( armas[arma_actual].autodisparo )
			...
			if ( jkeys_state[ _JKEY_L         ] and permitido)
				permitido = false;
		...
		// controla los disparos individuales
			...
			if ( jkeys_state[ _JKEY_L         ] and permitido and retraso_disparo > armas[arma_actual].retraso_manual )
				permitido = false;
				retraso_disparo = 0;
...
begin
	...
	loop
		...
		// al chocar con zombie atualiza las cariables publicas
...
begin
	loop
		if ( jkeys_state[ _JKEY_R         ] and posible and retraso > 10 and partida.granadas > 0)
			posible = false;
			retraso = 0;
			partida.granadas--;
		...
		if (not jkeys_state[ _JKEY_R         ] and posible == false)
			posible = true;
		end
