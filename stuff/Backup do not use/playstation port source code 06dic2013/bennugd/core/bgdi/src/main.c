...
int LoadSaveMC(char *filename, void *data, int len)
{
  	int mc_fd;
  	char cad[30];
	...
	// read  from the memory card (Note that this filename is fixed)
	mc_fd = fioOpen(cad,O_RDONLY);
	if(mc_fd <= 0) {
	  printf("MEMORY CARD load ERROR .\n");
	  return 0;
	}

	if(fioRead(mc_fd, data, len) != len) return 0;
	fioClose(mc_fd);

	printf("MEMORY CARD load sucessfully.\n");
	return 1;
}
