...
printf("Fase sysproc 3 \n");
 ...
 printf("Fase sysproc 4 \n");
    }
printf("Fase sysproc 5 \n");
    if ( debug ) printf ("\n");

    /* System Procs FixUp */

    sysprocs_fixup();
printf("Fase sysproc 6 \n");
    proc = sysprocs ;
    while ( proc->func )
    {
 ...
 printf("Fase sysproc 6.1 \n");
    /* Initialize all modules */
    if ( module_initialize_count )
        for ( n = 0; n < module_initialize_count; n++ )
        {  // if (n==9) continue;
            printf("Fase sysproc 6.2 %d \n",n);
            filename = string_get( dcb.imports[n] ) ;
            printf ("Loading... %s\n", filename );

            module_initialize_list[n]();
            printf("Fase sysproc 6.2 end \n");
        }

 printf("Fase sysproc 7 \n");
}
